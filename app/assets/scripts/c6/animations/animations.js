...
/*  ==========================================================================
	...
	/* --- show ---  */
		.animation('video-show', ['$log', 'c6AniCache', function($log, c6AniCache) {
			return c6AniCache({
				id: 'video-show',
				setup: function(element) {
...
/*  ==========================================================================
	...
	/* ---- panels close ---- */
	/*	.animation('landing=>experience-enter', ['$log', 'c6AniCache', function($log, c6AniCache) {
			var timelineGlobal;
			return c6AniCache({
				id: 'panels-close',
				setup: function(partial) {
					$log.log('In panels-close setup');
					partial.find('.transition__panels-top').css({'bottom': '50%'});
					partial.find('.transition__panels-bottom').css({'top': '50%'});	
				},
				start: function(partial, done) {
					$log.log('In panels-close start');
					var panelTop = partial.find('.transition__panels-top'),
						panelBottom = partial.find('.transition__panels-bottom'),
						timeline = new TimelineLite({paused: true});

					timeline.delay(0.6).to([panelTop, panelBottom], 0, {display: 'block'})
						.to(panelTop, 0.6, {bottom: '100%', ease: Power2.easeInOut}, '+=.6')
						.to(panelBottom, 0.6, {top: '100%', ease: Power2.easeInOut}, '-=0.6')
						.call(done, null, null, '+=.3')
						.to([panelTop, panelBottom], 0, {display: 'none'})
						.eventCallback('onComplete',function(){
							$log.info('panels-close done');
						});
					timelineGlobal = timeline;
					timeline.play();
				},
				cancel: function(partial, done) {
					$log.log('In panels-close cancel');
					timelineGlobal.kill();
					timelineGlobal.clear();
					done();
				}
			});
		}])
	...
	/* ---- panels open ---- */
	/*	.animation('landing=>experience-leave', ['$log', 'c6AniCache', function($log, c6AniCache) {
			var timelineGlobal;
			return c6AniCache({
				id: 'panels-open',
				setup: function(partial) {
					$log.log('In panels-open setup');
					partial.find('.transition__panels-top').css({bottom:'100%'}).show();
					partial.find('.transition__panels-bottom').css({top:'100%'}).show();
				},
				start: function(partial, done) {
					$log.log('In panels-open start');
					var panelTop = partial.find('.transition__panels-top'),
						panelBottom = partial.find('.transition__panels-bottom'),
						timeline = new TimelineLite({paused: true});

					timeline.to(panelTop, 0.6, {bottom: '50%', ease: Power2.easeInOut})
						.to(panelBottom, 0.6, {top: '50%', ease: Power2.easeInOut}, '-=0.6')
						.call(done, null, null, '+=0.3')
						.to([panelTop, panelBottom], 0, {display: 'none'})
						.eventCallback('onComplete',function(){
							$log.info('panels-open done');
						});
					timelineGlobal = timeline;
					timeline.play();
				},
				cancel: function(partial, done) {
					$log.log('In panels-open cancel');
					timelineGlobal.kill();
					timelineGlobal.clear();
					done();
				}
